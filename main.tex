...
\begin{document}
 ...
 \section{\makebox[\widthof{\faWrench}][c]{\color{NPU_Blue}{\faWrench}}\quad 技能特长}
    \vspace{0.3em}
    \begin{itemize}
    \item 熟练使用 Java，go 语言，熟悉 Java内存模型，深入了解 ThreadLoacl底层原理。
   ...
   \hfill 2025年3月-2025年6月
    \begin{itemize}
        \item 基于论文MapReduce设计分布式计算框架，通过临时文件+原子重命名保障任务执行的一致性，支持容错与动态扩缩容。  
        \item 实现 raft 算法的中的 \textbf{leader 选举，日志复制，持久化，快照}等，同时使用 \textbf{二分法} 优化日志回退，减少日志回退时间。
        \item 基于Raft构建线性一致状态机，通过\textbf{全局唯一ID(雪花算法)}保障指令有序执行。
    \end{itemize}
    ...
    \begin{itemize}
        \item 基于Bencode编码解析.torrent文件，提取Tracker地址、Piece分块（SHA1校验）、文件目录结构等元信息, 实现BitTorrent协议握手流程。
        \item 基于\textbf{channel}实现生产者消费者模型，并发下载内容。
        \item 基于\textbf{Range}实现断点续传，支持并发请求，实现 Http 的多线程下载
        \item 使用\textbf{mmap}内存映射技术，将文件直接映射至用户空间，绕过内核缓冲区（Page Cache），减少write()系统调用次数
    \end{itemize}
